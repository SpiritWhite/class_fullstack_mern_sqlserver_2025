...
import { selectCaughtPokemons, selectCurrentPage, selectError, selectLoading, selectPokemons } from './redux/selector/selector';
import { catchPokemon, setCurrentPage, setError, setLoading, setPokemons } from './redux/reducer/pokemonSlice';
...
function App() {
  ...
  useEffect(() => {
    const fetchPokemons = async () => {
      try {
        ...
        dispatch(setError('Error al cargar los pokémones'));
        dispatch(setLoading(false));
  ...
  const onAddPag = () => dispatch(setCurrentPage(currentPage + 1));

  const onRmvPag = () => dispatch(setCurrentPage(currentPage - 1));

  const handlePokemon = (pokemonId) => () => navigate(`/pokemon/${pokemonId}`);
